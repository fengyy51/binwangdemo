...
//在这个div里面画地图
  ...
  function bindEvent() {
    ...
    function onPan(ev) {
     ...
     console.log(pos);
        ...
        }else if((window.innerHeight-pos.bottom)-(window.innerHeight-document.getElementsByClassName("bianjie")[0].getBoundingClientRect().bottom+$(".bianjie").height())>=0){//向上移动判断
            var currentY;
  ...
  function moveMapTo(x, y) {
    ...
    START_X = clientmidx - x*delta;
    START_Y = clientmidy - y*delta;
    resetElement();
  }
  //开启地图中心随点击位置移动
